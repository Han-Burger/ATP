...
def player_rank_history_parser(url):
    ...
    return [[parse_table(d.text.strip()) for d in row.find_all('td')] for row in tb]
...
if __name__ == '__main__':
    ...
    mat = player_rank_history_parser(url)
    print(pd.DataFrame.from_records(mat, columns = rank_history_header))
